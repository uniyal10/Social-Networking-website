    ...
    switch (action.type) {
      case "usernameImmediately":
        ...
        if (draft.username.value && !/^([a-zA-Z0-9]+)$/.test(draft.username.value)) {
          draft.username.hasErrors = true
          draft.username.message = "special character are not allowed"
        }
        return
      case "usernameAfterDelay":
        if (draft.username.value.length < 3) {
          draft.username.hasErrors = true
          draft.username.message = "username must to be atleast 3 characters"
        }
      ...
      case "emailAfterDelay":
        if (!/^\S+@\S+$/.test(draft.email.value)) {
          ...
          draft.email.message = "you have to provide a valid email address"
        }
      ...
      case "emailUniqueResults":
        if (action.value) {
          ...
          draft.email.message = "That email already been used"
        } else {
      ...
      case "userAlreadyExist":
        draft.userAlreadyExist.hasErrors = true
        draft.userAlreadyExist.message = "user Already Exist"
    }
  }
    ...
    if (state.submitCount) {
      ...
      async function fetchResults() {
        try {
          ...
          dispatch({ type: "userAlreadyExist" })
        }
      }
      fetchResults()
      return () => ourRequest.cancel()
        ...
        <div className="col-lg-5 pl-lg-5 pb-3 py-lg-5">
          <form onSubmit={handleSubmit}>
            <div className="form-group">
              ...
              <CSSTransition in={state.userAlreadyExist.hasErrors} timeout={330} classNames="liveValidateMessage" unmountOnExit>
                <div className="alert alert-danger small liveValidateMessage">{state.userAlreadyExist.message}</div>
              </CSSTransition>
            </div>
            <div className="form-group">
